...
Twitter.prototype.stream = function(method, params, callback) {
	...
	if (method === 'user') {
		...
		// @params should look like 'track=%23hashtag1,%23hashtag2', etc
		get_params = '?' + params;
		params = null; // when performing a POST request, the params within body cause a 401 Authorization error
	}


	// var url = stream_base + '/' + escape(method) + '.json';
	var url = stream_base + '/' + escape(method) + '.json' + get_params;
